...
#Write function
distribution.function.primary.motivation <- function(cnmi.data.cleaned, breakdown){
  
  
  #Rename argument for group_by to work below
  cnmi.pri.mot.cleaned <- cnmi.data.cleaned %>% 
    rename(brk.down = breakdown)
  
  
  #Distribution calculation
  dist.primary.motivation <- cnmi.pri.mot.cleaned %>%
