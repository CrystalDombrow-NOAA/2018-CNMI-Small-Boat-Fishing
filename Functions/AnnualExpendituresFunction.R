...
#Write function
annual.expenditures.function <- function(cnmi.data.cleaned, breakdown){
  
  
  #Rename argument for group_by to work below
  cnmi.annual.exp.cleaned <- cnmi.data.cleaned %>% 
    rename(brk.down = breakdown)
  
  #Set up dataframe for calculations below
  annual.expenditures.colnames <- cnmi.annual.exp.cleaned %>%
