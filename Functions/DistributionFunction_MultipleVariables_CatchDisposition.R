...
#Write function
distribution.function.catch.disposition <- function(cnmi.data.cleaned, breakdown){
  
  
  #Rename argument for group_by to work below
  cnmi.catch.dis.cleaned <- cnmi.data.cleaned %>% 
    rename(brk.down = breakdown)
  
  
  #Distribution calculation
  dist.catch.disposition <- cnmi.catch.dis.cleaned %>%
