...
#Write function
data.summaries.function <- function(cnmi.data.cleaned, q.number){
  
  #Rename argument for group_by to work below
  cnmi.cleaned.data.sum.fun <- cnmi.data.cleaned %>% 
  ...
  q.data.sum.full.sample <- cnmi.cleaned.data.sum.fun %>%
  ...
  #Calculate data summaries
  q.data.sum.island <- cnmi.cleaned.data.sum.fun %>%
    select(any_of(c("q.num", "Island"))) %>%
    drop_na() %>%
    group_by(Island) %>%
    mutate(mean = round(mean(q.num, na.rm = T), 1),
           std.err = round(std.error(q.num, na.rm = T), 1),
  ...
  #Table
    #Set up rows for n
  q.saipan.rows <- q.data.sum.island %>%
    filter(Island == "Saipan") %>%
    drop_na() %>%
    nrow() 
  
  q.tinian.rows <- q.data.sum.island %>%
    filter(Island == "Tinian") %>%
    drop_na() %>%
    nrow()
  
    #Create data frame to later rbind into a table
  q.island.sum <- q.data.sum.island %>%
    mutate(n = (ifelse(Island == "Saipan", q.saipan.rows, q.tinian.rows))) %>%
    mutate(breakdown = Island, .before = n)
  
    #Remove unneeded rows and select unique breakdown values
  q.island.sum <- q.island.sum [-c(1:2)] %>%
    unique()
  ...
  #Calculate data summaries
  q.data.sum.island.2 <- cnmi.cleaned.data.sum.fun %>%
    select(any_of(c("q.num", "Island"))) %>%
    drop_na() %>%
    group_by(Island) %>%
    mutate(mean = round(mean(q.num, na.rm = T), 1),
  ...
  #Calculate number of observations for table
  q.rota.rows <- q.data.sum.island.2 %>%
    filter(Island == "Rota") %>%
    drop_na() %>%
    nrow()
  
  #Create data frame to later rbind into a table
  q.island.sum.2 <- q.data.sum.island.2 %>%
    mutate(n = q.rota.rows, .before = mean) %>%
    mutate(breakdown = Island, .before = n)
  
  #Remove unneeded rows and select unique breakdown values
  q.island.sum.2 <- q.island.sum.2[-c(1:2)] %>%
    unique()
  ...
  #Calculate data summaries
  q.data.sum.sell.fish <- cnmi.cleaned.data.sum.fun %>%
  ...
  #Calculate data summaries
  q.data.sum.highliner <- cnmi.cleaned.data.sum.fun %>%
  ...
  #Calculate data summaries
  q.data.sum.pri.target <- cnmi.cleaned.data.sum.fun %>%
  ...
  #Calculate data summaries
  q.data.sum.pri.target.2 <- cnmi.cleaned.data.sum.fun %>%
    select(any_of(c("q.num", "primary.target"))) %>%
  ...
  #Calculate data summaries
  q.data.sum.boat.owner <- cnmi.cleaned.data.sum.fun %>%
  ...
  column.order <- c("V1", "V3", "V2", "V4", "V9", "V10", "V11", "V8", "V12", "V13",
                    "V14", "V15", "V17", "V16")
  ...
  q.data.sum.list <- list("q.data.sum.full.sample" = q.data.sum.full.sample,
                          "q.data.sum.island" = q.data.sum.island,
                          "q.data.sum.island.2" = q.data.sum.island.2,
