...
trip.costs.function <- function(gear.type.trip, breakdown){
  ...
  cnmi.cleaned.trip.costs <- cnmi.data.cleaned %>% 
  ...
  primary <- cnmi.cleaned.trip.costs %>%
  ...
  secondary <- cnmi.cleaned.trip.costs %>%
  ...
  #Set up dataframe for primary gear so can rbind later
  q36.39.primary.occ <- cnmi.cleaned.trip.costs %>%
  ...
  #Set up dataframe for secondary gear so can rbind later
  q36.39.secondary.occ <- cnmi.cleaned.trip.costs %>%
  ...
  trip.costs.primary.occ <- cnmi.cleaned.trip.costs %>%
    select(Q34A.combined, Q36H, Q36I, brkdwn) %>%
    filter(Q34A.combined == gear.type.trip) %>%
    drop_na() %>%
    group_by(brkdwn) %>%
  ...
  trip.costs.secondary.occ <- cnmi.cleaned.trip.costs %>%
    select(Q37A.combined, Q39H, Q39I, brkdwn) %>%
    filter(Q37A.combined == gear.type.trip) %>%
    drop_na() %>%
    group_by(brkdwn) %>%
