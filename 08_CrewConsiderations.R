...
#DISTRIBUTION
q13a.distr <- distribution.function(cnmi.data.cleaned, q.number = "Q13A", 
                                    categories = 1:6)

q13b.distr <- distribution.function(cnmi.data.cleaned, q.number = "Q13B", 
                                    categories = 1:6)


#DATA SUMMARIES
q13a.data.sum <- data.summaries.function(cnmi.data.cleaned, q.number = "Q13A.mid")

q13b.data.sum <- data.summaries.function(cnmi.data.cleaned, q.number = "Q13B.mid")
...
q14.data <- cnmi.data.cleaned %>% 
  select(Q13B, Q14A:Q14F, Q14.multiple.responses) %>%
  filter(Q13B > 4)

q14.percents <- cnmi.data.cleaned %>% 
  select(Q13B, Q14A:Q14F, Q14.multiple.responses) %>%
  filter(Q13B > 4) %>% #Over 50% of the time fishing as crew from Q13B
