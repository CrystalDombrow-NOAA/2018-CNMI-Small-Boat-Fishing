#------------------------------------------------------------------------------
#PURPOSE: This script cleans the data for the 2018 CNMI small boat 
...
create.data.groups <- recode.more.variables %>%
  ...
  mutate(boat.owner = ifelse(Q24A == 1 & Survey != "1190B", "boat owner", "not boat owner"),
         ...
         highliner.pelagic = ifelse(sell.fish == 1 & Q8A.mid.ifelse >= 5000, 1, 0),
         highliner.bf = ifelse(sell.fish == 1 & Q8B.mid.ifelse >= 2500, 1, 0),
         highliner.reef = ifelse(sell.fish == 1 & Q8C.mid.ifelse >= 500, 1, 0),
...
#Select CNMI observations for the CNMI report.
cnmi.data.cleaned <- guam.cnmi.data.cleaned %>%
  filter(Island != "Guam")

#Save as a CSV.
write_csv(cnmi.data.cleaned, "Data/CNMI_SBF_2018_DataCleaned.csv")
