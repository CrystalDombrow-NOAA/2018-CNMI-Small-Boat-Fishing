...
#pelagics (n = 57), top 10%: 5000-10,000 lbs (n = 11)
pelagics.highliner <- cnmi.data.cleaned %>%
  select(Survey, Q8A.mid.ifelse) %>%
  slice_max(order_by = Q8A.mid.ifelse, prop = 0.1)

#bottomfish (n = 60), top 10%: 2500-5000 lbs (n = 6)
bottomfish.highliner <- cnmi.data.cleaned %>%
  select(Survey, Q8B.mid.ifelse) %>%
  slice_max(order_by = Q8B.mid.ifelse, prop = 0.1)

#reef fish (n = 55), top 10%: 500-2000 lbs (n = 6)
reef.highliner <- cnmi.data.cleaned %>%
  select(Survey, Q8C.mid.ifelse) %>%
  slice_max(order_by = Q8C.mid.ifelse, prop = 0.1)

highliners.total <- cnmi.data.cleaned %>%
